...
for(t in taxids){
  ...
  current_genome <- ggplot() + 
    ...
    geom_text(data=Anytaxon_df_UTRs, aes(x=c(max(Anytaxon_df$End)+(max(Anytaxon_df$End)/30),-(max(Anytaxon_df$End)/30)), y=0, label=gsub("_","' ",Region_feature_class)), size=3)+
