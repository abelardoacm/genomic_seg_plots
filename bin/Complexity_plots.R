...
#############################################################
taxids <- tabla_features$NCBI_taxid[!duplicated(tabla_features$NCBI_taxid)] # Make a vector containing unique taxids
...
for(t in taxids){
  ...
  if(length(Anytaxon_df_UTRs$Viral_species)==0){
    Anytaxon_df_UTRs <- Anytaxon_df_UTRs %>% add_row(Viral_species = Anytaxon_df$Viral_species[1], NCBI_taxid=Anytaxon_df$NCBI_taxid[1], Region_feature_class="UTR", Beginning=0,End=0)
    Anytaxon_df_UTRs <- Anytaxon_df_UTRs %>% add_row(Viral_species = Anytaxon_df$Viral_species[1], NCBI_taxid=Anytaxon_df$NCBI_taxid[1], Region_feature_class="UTR", Beginning=0,End=0)
  }
  ...
  if(length(Anytaxon_df_complexity$viral_species)==0){
    Anytaxon_df_complexity <- Anytaxon_df_complexity %>% add_row(viral_species= Anytaxon_df$Viral_species[1], ncbi_taxid= Anytaxon_df$NCBI_taxid[1], origin_beg= 0, origin_end= 1, codon_start= 1, product= "none", seg_begin= 0, seg_end= 1, complexity= 0, window= 0, locut= 0, hicut= 0, secuence="", length_in_aa=0, length_in_nc=0,adj_beg=0,adj_end=0)
  }
  ...
  tiff(paste("../results/Complexity_genomic_plots/",anytaxon,"/",outfilename,sep=""), units="in", width=7, height=5, res=600) #tiff resolution parameters
  current_genome <- ggplot() + 
    ...
    geom_text(data=Anytaxon_df_peptides, aes(x=Beginning+(End-Beginning)/2, y=head(alternate_v,length(End))/6, label=str_wrap(product, width = 10)), size=2.8)+
    theme(legend.position="none")+
    theme(axis.title.y=element_blank(),
          axis.text.y=element_blank(),
          axis.ticks.y=element_blank())+
    geom_rect(data=Anytaxon_df_UTRs, mapping=aes(xmin=Beginning, xmax=End, ymin=-0.1, ymax=0.1), alpha=0.5)+
    geom_text(data=Anytaxon_df_UTRs, aes(x=c(max(End)+(max(End)/50),-(max(End)/50)), y=0, label=gsub("_","' ",Region_feature_class)), size=3)+
    ...
    geom_text(data=Anytaxon_df_genes, aes(x=Beginning+(End-Beginning)/2, y=seq(from = -0.5,by = -0.2,length.out = length(gene)), label=str_wrap(gene, width = 10)), size=2.8)+
    geom_segment(data=Anytaxon_df_stemloops, mapping=aes(x=Beginning,y=0, xend=End , yend=0), color="blue", size=3)+
    geom_rect(data=Anytaxon_lo_complexity, mapping=aes(xmin=adj_beg, xmax=adj_end, ymin=normalized_complexity, ymax=mean(Anytaxon_df_complexity$normalized_complexity),fill="blue"), alpha=0.5)
