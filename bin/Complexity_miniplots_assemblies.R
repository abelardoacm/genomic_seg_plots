...
#MINIMUM PLOTS
# ITERATE OVER EVERY UNIQUE NAME TO BUILD MINIPLOT.tiff
for(t in virus_species){
  i = i +1
  Anytaxon_df <- tabla_features[which(tabla_features$Viral_species == t),]
  ...
  Anytaxon_df_complexity <- complexity_table[which(complexity_table$viral_species == t),]
  ...
  outfilename = paste(Anytaxon_df$Viral_species[1],"id",Anytaxon_df$NCBI_taxid[1],"miniplot")
  ...
  current_genome <- ggplot() + 
    geom_segment(data = Anytaxon_df_peptides , aes(x = Beginning, y = 0, xend = Beginning, yend = 0), colour = "gray80",size=0.02)+
    theme_bw() + 
    ...
    geom_segment(aes(x = 0, y = 0, xend = (1.5*max(Anytaxon_df$End, na.rm = TRUE)), yend = 0), color = "white",size=0.1)+
    ...
    geom_text(hjust="outward",data=Anytaxon_df, aes(x=max(Anytaxon_df$End), y=0, label=paste(" ",gsub("[^a-zA-Z0-9]"," ",Anytaxon_df$isolate))),color="black" ,size=1.5)+
    geom_point(data=Anytaxon_lo_complexity, mapping=aes(x=((adj_beg + adj_end)/2), y=0),shape=25, fill="deeppink", color="deeppink", size=1.2)
...
#taxids <- complexity_table$ncbi_taxid[!duplicated(complexity_table$ncbi_taxid)]
...
features_file_with_ref <- system(paste("grep -c ",t, "../results/GenFeatures_locations/*.csv | grep -v assemblies | cut -d: -f1"), intern = TRUE)
complexity_file_with_ref <- system(paste("grep -c ",t, "../results/seg/*.csv | grep -v assemblies | cut -d: -f1"), intern = TRUE)
...
tiff("stackedplotref.tiff", units="in", width=10, height=5, res=600) #tiff resolution parameters
